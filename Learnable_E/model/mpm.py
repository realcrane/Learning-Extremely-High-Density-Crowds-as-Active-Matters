...
from model.MyParaNetwork import Parameter_Estimate
...
class MPM(nn.Module):
    ...
    def forward(self):
        ...
        return torch.stack(self.grid_v_out_seq), torch.stack(self.grid_v_in_seq)
    ...
    def P2G(self, ):
        ...
        momentum_temp = w_momentum + w_angle_momentum #+ w_elastic_force + w_external_force
        self.grid_v_in[mask, :] = torch.div(momentum_temp[mask, :], self.grid_mass[mask].unsqueeze(-1))

        self.grid_momentum = w_momentum + w_angle_momentum + w_elastic_force + w_external_force
