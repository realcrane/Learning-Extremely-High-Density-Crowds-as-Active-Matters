...
class DataProcess:
    def __init__(self, ):
        self.pixel_vel = ti.Vector.field(dim, dtype=ti.f32, shape=(res[0], res[1]))
    ...
    def pixel_to_field(self, vel_tensor: ti.template(), vel_field: ti.template(), mass_field: ti.template()):
        for p in ti.grouped(ti.ndrange(res[0], res[1])):
            if self.is_inside(p):
                Xp = p * inv_dx
    ...
    def is_inside(self, p):
        flag = p[0] >= pixel_bound and p[0] < (res[0] - pixel_bound) and p[1] >= pixel_bound + 10 and p[1] < (
                res[1] - pixel_bound)
        return flag
    ...
    def load_optical_flow(self, frame, data_path):
        ...
        o_flow_x = o_flow_x * 25
        o_flow_y = o_flow_y * 25
        temp = np.zeros([res[0], res[1], 2])
        temp[pixel_bound:res[0] - pixel_bound, pixel_bound + 10:res[1] - pixel_bound, 0] = np.transpose(o_flow_x)
        temp[pixel_bound:res[0] - pixel_bound, pixel_bound + 10:res[1] - pixel_bound, 1] = np.transpose(o_flow_y)
