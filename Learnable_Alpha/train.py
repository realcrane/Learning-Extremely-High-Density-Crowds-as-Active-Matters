...
def train(model, optimizer, scheduler, train_loader, cfg, epoch):
    model.train()
    model.get_super_paras_E.eval()
    model.get_super_paras_K.eval()
    model.n_substeps = cfg.n_substeps
    loss = Metric('mseloss')

    # Start batch training
    infos = 'Train {} epoch {}/{}'.format('MPM only Alpha', epoch, cfg.epochs)
...
def val(model, val_loader, cfg, epoch):
    ...
    with torch.no_grad():
        # Start batch training
        infos = 'Val {} epoch {}/{}'.format('MPM only K', epoch, cfg.epochs)
...
if __name__ == '__main__':
    ...
    root_folder = f'checkpoints_E{cfg.E}_w_ext{cfg.w_ext}_K{cfg.K}_alpha'
    ...
    model_path = '../LearnableK/checkpoints_E2000_w_ext0.03_K3/exp1_lr0.0001_lambda0.9_r5/mpm_best.pth'
    if os.path.exists(model_path):
        ...
        print(f'the epoch is {st_epoch}, loss: {best_loss}')
        ...
        log_file.log('loaded the get_super_paras_E')
        ...
        for ind, key in enumerate(state_dict):
            if 'get_super_paras_K' in key:
                ...
                state_dict_new[key_new] = state_dict[key]
        model.get_super_paras_K.load_state_dict(state_dict_new)
        log_file.log('loaded the get_super_paras_K')
    ...
    optimizer = torch.optim.Adam(model.get_super_paras_alpha.parameters(), lr=cfg.lr)
    ...
    for epoch in range(cfg.st_epoch, cfg.epochs):
        ...
        log_file.log('Alpha:{}'.format(model.alpha.view(-1).detach().cpu().numpy()), flag=False)
