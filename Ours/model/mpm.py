...
class MPM(nn.Module):
    def __init__(self, cfg):
        ...
        self.get_active_force2 = CVAE(z_dim=64, n_decoder=cfg.n_decoder)
    ...
    def P2G(self, ):
        ...
        # calculate the second term, take |v|^2v, dv/dx, z as input
        cond_1 = torch.sum(grid_v_temp ** 2, dim=-1, keepdim=True) * grid_v_temp  # |v|^2v

        cond_2 = torch.zeros_like(grid_v_temp).to(grid_v_temp.device)
        temp = torch.zeros(self.cfg.n_grid[0], self.cfg.n_grid[1], 1).to(grid_v_temp.device)
        dvx_dx = (grid_v_temp[2:30, 1:19, 0] - grid_v_temp[0:28, 1:19, 0]) / (2 * self.cfg.dx)
        dvy_dy = (grid_v_temp[1:29, 2:20, 1] - grid_v_temp[1:29, 0:18, 1]) / (2 * self.cfg.dx)
        ...
        cond_3 = torch.zeros_like(grid_v_temp).to(grid_v_temp.device)
        cond_3[1:29, 1:19, 0] = (grid_v_temp[2:30, 1:19, 0] + grid_v_temp[0:28, 1:19, 0] - grid_v_temp[1:29, 1:19, 0])
        cond_3[1:29, 1:19, 1] = (grid_v_temp[1:29, 2:20, 1] + grid_v_temp[1:29, 0:18, 1] - grid_v_temp[1:29, 1:19, 1])
        cond_3 = cond_3 / self.cfg.dx ** 2

        cond_4 = torch.zeros_like(grid_v_temp).to(grid_v_temp.device)
        dvx_dx = (grid_v_temp[2:30, 1:19, 0] - grid_v_temp[0:28, 1:19, 0]) / (2 * self.cfg.dx)
        dvy_dy = (grid_v_temp[1:29, 2:20, 1] - grid_v_temp[1:29, 0:18, 1]) / (2 * self.cfg.dx)
        dvx_dy = (grid_v_temp[1:29, 2:20, 0] - grid_v_temp[1:29, 0:18, 0]) / (2 * self.cfg.dx)
        dvy_dx = (grid_v_temp[2:30, 1:19, 1] - grid_v_temp[0:28, 1:19, 1]) / (2 * self.cfg.dx)
        cond_4[1:29, 1:19, 0] = grid_v_temp[1:29, 1:19, 0] * dvx_dx + grid_v_temp[1:29, 1:19, 1] * dvx_dy
        cond_4[1:29, 1:19, 1] = grid_v_temp[1:29, 1:19, 0] * dvy_dx + grid_v_temp[1:29, 1:19, 1] * dvy_dy

        input = torch.cat([cond_1, cond_2, cond_3, cond_4], dim=-1).unsqueeze(0).permute(0, 3, 1, 2)
        mask = torch.zeros(30, 20).bool().cuda()
        mask[3:27, 3:17] = True
        input[:, :, ~mask] = 0
        output = self.get_active_force2.sample(input).permute(1, 2, 0)
        output[~mask, :] = 0
        output = output.view(-1, 2) * 1 / 50
        w_active_force_2 = output * self.grid_mass.unsqueeze(1)

        self.grid_momentum = w_momentum + w_angle_momentum + w_elastic_force + w_external_force + w_defined_force + w_active_force_1 + w_active_force_2

    def OP(self):
        mask = self.grid_mass > 0
        self.grid_v_out[mask, :] = torch.div(self.grid_momentum[mask, :], self.grid_mass[mask].unsqueeze(-1))
        # temp = self.grid_v_out.view([30, 20, 2])
        # print(temp[3:27, 3:17, :].abs().max())
        # print('--------')
